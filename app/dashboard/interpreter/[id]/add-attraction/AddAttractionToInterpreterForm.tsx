...
export default function AddAttractionPage({interpreterId}: {interpreterId: number}) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.push(`/dashboard/interpreter/${interpreterId}`);
      router.refresh();
    } catch (error) {
  ...
  if (error) {
    return (
      <div className="container py-5">
        ...
        <Link href={`/dashboard/interpreter/${interpreterId}`} className="btn btn-secondary">
          Back to Interpreter Details
        </Link>
      </div>
    );
  }
