...
export default function EditAttractionForm({ attractionId }: { attractionId: number }) {
  ...
  useEffect(() => {
    const loadAttractionData = async () => {
      try {
        setIsLoading(true);
        setError(null);
        const attractions = await fetchAttractionById(attractionId);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.push(`/dashboard/attraction/${attractionId}`);
      router.refresh();
    } catch (error) {
  ...
  if (error) {
    return (
      <div className="container py-5">
        ...
        <Link href={`/dashboard/attraction/${attractionId}`} className="btn btn-secondary">
